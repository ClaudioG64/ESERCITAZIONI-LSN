  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 5.0 * sqrt(_temp) / double(_n_bins_v); //MODIFICATO PER ES 4.1 (5*SQRT(TEMP))
        _measure_pofv = true;
        ...
        index_property++;
      } else if( property == "ENDPROPERTIES" ){   //se legge endproperties chiude la questione
        ofstream coutf;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(2, this->pbc(_side(2)*z / 2, 2));  // HO MESSO IN X,Y,Z "/2" PER ESERCIZIO 4.2 CHE CHIEDE MEZZA BOX
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_pofv) {
    for (int i=0; i<_npart; i++) {
        // Calcolo del modulo della velocità della particella i
...
void System :: averages(int blk){
  ...
  if (_measure_pofv) {
    ofstream coutpv("../OUTPUT/pofv.dat", ios::app);
    for (int k=0; k<_n_bins_v; k++) {
        double v_center = (k+0.5) * _bin_size_v;

        // Normalizza i conteggi in densità
        double norm_factor = double(_npart) * _bin_size_v;
        // dove _nconf è il numero di configurazioni misurate per blocco (nel codice NSL di solito è già disponibile!)

        double average     = _average(_index_pofv + k) / norm_factor;
        double sum_average = _global_av(_index_pofv + k) / double(blk) / norm_factor;
        double sum_ave2    = _global_av2(_index_pofv + k) / double(blk) / (norm_factor*norm_factor);
        double err         = error(_global_av(_index_pofv + k)/norm_factor,
                           _global_av2(_index_pofv + k)/(norm_factor*norm_factor),
                           blk);


        coutpv << setw(12) << v_center
               << setw(12) << average
               << setw(12) << sum_average
               << setw(12) << err << endl;
    }
