...
TSPGA::TSPGA(int populationSize, bool useL1, int rank)
    : populationSize(populationSize), useL1(useL1),
    ...
    int seed[4];
    int p1 = 0, p2 = 0;
    std::time_t now = std::time(0) + rank;
...
void TSPGA::setCities(const std::vector<City>& cities_list) {
    cities = cities_list;
...
void TSPGA::mutateShift(Individual& ind) {
    if (randomDouble(0.0, 1.0) < pMutationShift) {
        ...
        int shift = randomInt(1, numCities - start - length - 1);
...
void TSPGA::mutateSwap(Individual& ind) {
    if (randomDouble(0.0, 1.0) < pMutationSwap) {
        ...
        int attempts = 0;
        while (std::abs(start2 - start1) < length1 && attempts < max_attempts) {
...
void TSPGA::replaceWorstIndividual(const Individual& ind) {
    if (populationSize > 0 && checkIndividual(ind)) {
        population[populationSize-1] = ind;
        population[populationSize-1].fitness = calculateFitness(population[populationSize-1]);
        // Riordina la popolazione dopo la sostituzione
        std::sort(population.begin(), population.end(), 
                  [](const Individual& a, const Individual& b) {
                      return a.fitness < b.fitness;
                  });
...
double TSPGA::getAverageFitness() const {
    double avg_fitness = 0.0;
    for (int i = 0; i < populationSize / 2; i++) {
        avg_fitness += population[i].fitness;
    }
    return avg_fitness / (populationSize / 2);
}