...
void System :: measure(){ 
  ...
  // total energy per spin (Ising)
  if (_measure_tenergy || _measure_cv){
    double s_i, s_j;
    for (int i=0; i<_npart; i++){
    ...
    if (_measure_cv)      _measurement(_index_cv)      = tenergy_temp * tenergy_temp; // <E²>
  }
  ...
  if (_measure_chi){
    ...
    _measurement(_index_chi) = spin_sum*spin_sum;
  }
