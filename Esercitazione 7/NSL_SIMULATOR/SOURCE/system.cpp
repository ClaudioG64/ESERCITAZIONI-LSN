...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ // Gibbs sampler ADDED FOR EXERCISE 6
    double sum_neigh = _particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin();
    double prob_up = 1.0 / (1.0 + exp(-2.0*_beta*( _J*sum_neigh + _H )));
    if(_rnd.Rannyu() < prob_up) _particle(i).setspin(1);
    else _particle(i).setspin(-1);
    _naccepted++; // ogni step è accettato
}
 else {           // M(RT)^2
    if(_sim_type == 1){       // Monte Carlo - LJ system
      vec shift(_ndim);       // Will store the proposed translation
      for(int j=0; j<_ndim; j++){
        shift(j) = _rnd.Rannyu(-1.0,1.0) * _delta; // uniform distribution in [-_delta;_delta)  EXERCISE 6, tweak delta to get 50% acceptance
      }
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      coutf << "VTail (per particle)= " << _vtail << ", PTail (stored)= " << _ptail << endl;
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    // vettori temporanei per le componenti delle velocità
    vec vx(_npart), vy(_npart), vz(_npart);
    vec sumv(_ndim);
    sumv.zeros();

    // --- Dirac delta initialization (exercise 4.02) ---
    // assegniamo a ciascuna particella una velocità con modulo v0 solo su una componente (vx)
    // v0 scelto in modo che v0^2 = 3 * T (in unità ridotte), quindi l'energia iniziale corrisponde alla T
    double v0 = sqrt(3.0 * _temp);

    for (int i=0; i<_npart; i++){
      // assegno vx = +v0 o -v0 a caso, vy = vz = 0
      double sign = (_rnd.Rannyu() < 0.5) ? -1.0 : 1.0;
      vx(i) = sign * v0;
      vy(i) = 0.0;
      vz(i) = 0.0;
    ...
    // rimuovo la velocita' del centro di massa
    ...
    // riscalo per avere esattamente la temperatura voluta (controllo dell'energia cinetica)
    double scalef = 1.0;
    if (sumv2 > 0.0) scalef = sqrt(3.0 * _temp / sumv2);
    ...
    // impostazione delle posizioni old (per l'integratore)
    for (int i=0; i<_npart; i++){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8*M_PI*_rho/(9*pow(_r_cut,9))-8*M_PI*_rho/(3*pow(_r_cut,3)); //tail correction for potential energy (fixed in ex 7)
        ...
        index_property++;
        _ptail = (16.0/3.0) * M_PI * _rho * ( (2.0/3.0) / pow(_r_cut,9) - 1.0 / pow(_r_cut,3) ); //tail correction for pressure (fixed in ex 7)
        ...
        coutchi << "#     BLOCK:   ACTUAL_X:     X_AVE:       ERROR:" << endl;
        coutchi.close();
        _nprop++;
        _measure_chi = true;
        _index_chi = index_property;
        index_property++;
      } else if( property == "GOFR" ){  //parsing for radial funct
        ofstream coutgr("../OUTPUT/gofr.dat");
        coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
        coutgr.close();
        input >> _n_bins;                       // numero di bin da file
        _nprop += _n_bins;
...
void System :: block_reset(int blk){
  ...
  _block_av.zeros();
  _nattempts = 0;   // reset tentativi
  _naccepted = 0;   // reset accettazioni
  return;
}
...
void System :: measure(){ 
  ...
  //   Lennard-Jones part
  // ==========================
  if (_sim_type < 2){  // MD or MC on LJ particles
    vec distance(_ndim);
    double dr;
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if (_measure_gofr){
          ...
          if (bin < _n_bins && bin >= 0){
            double norm = _rho*_npart*(4.0/3.0)*M_PI*( pow((bin+1)*_bin_size,3) - pow(bin*_bin_size,3) );
            _measurement(_index_gofr + bin) += 2.0 / norm; // 2 perché contiamo (i,j) e (j,i)
          }
        }
        // ===========================
    ...
    if (_measure_temp && _measure_kenergy){
  ...
  if (_sim_type >= 2){
    ...
    tenergy2_temp = pow(tenergy_temp,2);   // <E²>
    ...
    if (_measure_cv) 
      _measurement(_index_cv) = pow(_beta,2) * (tenergy2_temp - tenergy_temp*tenergy_temp);
    ...
    if (_measure_chi){
      double spin_sum=0;
      for (int i=0;i<_npart;i++) spin_sum += _particle(i).getspin();
      spin_sum /= double(_npart);
      _measurement(_index_chi) = _beta * spin_sum * spin_sum;
    }
  }
...
void System :: averages(int blk){
  ...
  if (_measure_chi){
    ...
    _global_av(_index_chi) += average - _average(_index_chi); 
    _global_av2(_index_chi) += average*average - _average(_index_chi)*_average(_index_chi);
  ...
  // specific heat
  if (_measure_cv){
    double e_ave  = _average(_index_tenergy);
    double e2_ave = _average(_index_cv);
    average = pow(_beta,2) * ( e2_ave - e_ave*e_ave ) * _npart;
  ...
  if (_measure_gofr){
    ...
    for (int i=0; i<_n_bins; i++){
      ...
      coutf << setw(12) << (i+0.5)*_bin_size   // r = centro del bin
            << setw(12) << average
            << setw(12) << sum_average/double(blk)
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
